    ...
    const fetchComplaints = async () => {
        try {
            const [pending, resolved] = await Promise.all([
                api.getPendingComplaints(),
                api.getAdminResolvedComplaints()
            ]);
        ...
        <div className="min-h-screen">
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p className="text-slate-500 mt-1">Manage maintenance requests from day scholars</p>
                </div>
            ...
            <div className="flex gap-2 mb-6">
                <button
                    ...
                    className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm uppercase tracking-wider transition-all ${activeTab === 'pending' ? 'bg-yellow-600 text-white shadow-lg shadow-yellow-900/40' : 'bg-slate-800/60 text-slate-400 hover:bg-slate-700 hover:scale-[1.02]'
                        }`}
                >
                    <Clock size={18} weight="duotone" /> Pending ({pendingComplaints.length})
                </button>
                <button
                    onClick={() => setActiveTab('history')}
                    className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm uppercase tracking-wider transition-all ${activeTab === 'history' ? 'bg-green-600 text-white shadow-lg shadow-green-900/40' : 'bg-slate-800/60 text-slate-400 hover:bg-slate-700 hover:scale-[1.02]'
                        }`}
                >
                    <ClockCounterClockwise size={18} weight="duotone" /> History ({resolvedComplaints.length})
                </button>
            </div>
            ...
            {complaints.length === 0 ? (
                <div className="bg-slate-800/30 border border-dashed border-slate-700 rounded-xl p-16 text-center relative overflow-hidden">
                    ...
                    <p className="text-slate-500">
                        {activeTab === 'pending' ? 'No pending complaints' : 'Resolved complaints will appear here'}
                    </p>
                </div>
            ) : (
                    ...
                    <div className="divide-y divide-slate-700/50">
                        {complaints.map((c) => {
                            ...
                            return (
                                <div
                                    key={c.id}
                                    className={`grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-slate-800/50 ${activeTab === 'pending' ? 'cursor-pointer' : ''
                                        }`}
                                    onClick={() => activeTab === 'pending' && setSelectedComplaint(c)}
                                >
                                    <div className="col-span-3">
                                        <p className="text-slate-200 line-clamp-1">{c.description}</p>
                                        <p className="text-xs text-slate-500 mt-0.5">{c.student_name || `#${c.student_id}`}</p>
                                    </div>
                                    ...
                                    <div className="col-span-2">
                                        <span className={`px-2 py-1 text-xs rounded ${priority.bg} ${priority.text}`}>
                                            {c.priority}
                                        </span>
                                    </div>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg shadow-2xl">
                        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700">
                            <h3 className="text-lg font-bold">Manage Complaint</h3>
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            {(() => {
                                ...
                                return (
                                    <div className={`px-4 py-2 rounded-lg ${status.bg} flex items-center justify-between`}>
                            ...
                            <div className="bg-slate-800/50 rounded-xl p-4 space-y-2">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className={`px-2 py-0.5 text-xs rounded ${PRIORITY_STYLES[selectedComplaint.priority]?.bg} ${PRIORITY_STYLES[selectedComplaint.priority]?.text}`}>
                                        {selectedComplaint.priority}
                                    </span>
                                </div>
                                <p className="text-sm text-slate-400 flex items-center gap-1">
                                    <MapPin size={14} /> {selectedComplaint.location}
                                </p>
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <button
                                            ...
                                            className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-900/30 border border-purple-700/50 rounded-xl hover:bg-purple-900/50 text-purple-300 text-sm disabled:opacity-50"
                                        >
                                            {suggestingNotes ? (
                                                <><div className="w-3.5 h-3.5 rounded-full border-2 border-purple-700 border-t-purple-300 animate-spin" /> Generating...</>
                                            ) : (
                                                <><MagicWand size={14} weight="duotone" /> AI Suggest</>
                                            )}
                                        </button>
                                    </div>
                                    <textarea
